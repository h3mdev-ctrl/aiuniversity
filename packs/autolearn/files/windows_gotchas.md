...
prompt from stdin when no prompt arg is given:

```python
subprocess.run([exe, "-p", "--model", model], input=prompt, capture_output=True, text=True)
```
...
## 5. Your memory folder needs to be a git repo for rollback to work
...
## 6. (Related) the credential guard can block your *commit message*
